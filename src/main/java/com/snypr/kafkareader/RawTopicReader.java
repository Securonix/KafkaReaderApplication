...
import java.util.*;
import com.securonix.kafkaclient.RawMessage;
import com.securonix.application.common.EventCompressor;
import org.apache.kafka.clients.consumer.ConsumerConfig;
import org.apache.kafka.common.serialization.StringDeserializer;
import org.apache.kafka.clients.consumer.ConsumerRecord;
import com.securonix.kafka.CustomAvroDecoder;
import java.io.IOException;

/**
 * This RawTopicReader class reads data from Raw Topic,and store corresponding
 * raw messages into a file.
 *
 * @author ManishKumar
 * @version 1.0
 * @since 2018-05-19
 */
public class RawTopicReader {
    ...
    public static void main(String args[]) {
        ...
        //RawTopic name is configured to read data.
        kafkaConfigBean.setRawTopic("alpha-Raw");
        ...
        final String groupId = "RawTopicReader-" + UUID.randomUUID().toString();
        ...
        try {
            ...
            KafkaUtil.addSSLPropertiesToConsumer(props, groupId, kafkaConfigBean, StringDeserializer.class.getName(), CustomAvroDecoder.class.getName());

            // Get Kafka Consumer.
            KafkaConsumer<String, RawMessage> consumer = KafkaUtil.getKafkaConsumer(kafkaConfigBean, props);

            // Subscribe Raw Topic for data poll.
            consumer.subscribe(Arrays.asList(kafkaConfigBean.getRawTopic()));

            // mList is used to store all JSON requests (fetched from enriched topic).
            ...
            while (true) {
                ...
                ConsumerRecords<String, RawMessage> records = consumer.poll(1000);
                ...
                for (ConsumerRecord<String, RawMessage> record : records) {
                    ...
                    if (event.isCompressed()) {
                        try {
                            ...
                            List<String> rawFeeds = EventCompressor.uncompress(event.getMessages(), event.getCompressionType());

                            // Add raw message list to mList,which will be used to write data into configured path
                            mList.addAll(rawFeeds);

                        } catch (Exception ex) {
                            LOGGER.error("Error uncompressing", ex);
                        }
                    } else {
                ...
                if (!mList.isEmpty()) {
                    ...
                    try {
                        ...
                        fstream = new FileWriter("/Users/manishkumar/Documents/manish/code/tempFolder/raw.txt", true);
                        ...
                        LOGGER.error("Failed to write raw input into file" + e);
                    } finally {
                        if (bw != null) {
                            try {
                                bw.close();
            ...
            LOGGER.error("Failed to read from Raw topic" + e);
        }
